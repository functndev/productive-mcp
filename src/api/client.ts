...
export class ProductiveAPIClient {
  ...
  private async makeRequest<T>(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          `API Error [${response.status}] ${options?.method ?? 'GET'} ${url}:`,
          JSON.stringify(errorData, null, 2),
        );
        const errorMessage = errorData.errors?.length
          ? errorData.errors.map(e => {
              const field = e.source?.pointer
   ...
   *
   * Per the Productive OpenAPI spec, /people supports filter[person_type] with
   * the underlying numeric IDs: 1 = user, 2 = contact, 3 = placeholder, 4 = agent.
   * (The string names from the spec description are rejected by the live API.)
   *
  ...
  async listUsers(): Promise<{
    ...
    while (true) {
      ...
      const response = await this.makeRequest<
        ProductiveResponse<ProductivePerson>
  ...
  async listFolders(params?: {
    ...
    const qs = q.toString();
    return this.makeRequest<ProductiveResponse<ProductiveFolder>>(
      `folders${qs ? `?${qs}` : ""}`,
    );
  }
  ...
  async getFolder(
    ...
    return this.makeRequest<ProductiveSingleResponse<ProductiveFolder>>(
      `folders/${folderId}`,
    );
  }
  ...
  async archiveTaskList(taskListId: string): Promise<void> {
    return this.makeVoidRequest(`task_lists/${taskListId}/archive`, {
      method: "PATCH",
    });
  }

  async restoreTaskList(taskListId: string): Promise<void> {
    return this.makeVoidRequest(`task_lists/${taskListId}/restore`, {
      method: "PATCH",
    });
  }
  ...
  async repositionTaskList(
    ...
    return this.makeVoidRequest(`task_lists/${taskListId}/reposition`, {
      ...
      body: JSON.stringify({
        data: {
          type: "task_lists",
          attributes: { move_before_id: moveBeforeId },
        },
      }),
    });
  }
  ...
  async listComments(params?: {
    ...
    if (params?.discussion_id)
      q.append("filter[discussion_id]", params.discussion_id);
    if (params?.page_id) q.append("filter[page_id]", params.page_id);
    if (params?.limit) q.append("page[size]", params.limit.toString());
    if (params?.page) q.append("page[number]", params.page.toString());
    const qs = q.toString();
    return this.makeRequest<ProductiveResponse<ProductiveComment>>(
      `comments${qs ? `?${qs}` : ""}`,
    );
  }
  ...
  async pinComment(commentId: string): Promise<void> {
    return this.makeVoidRequest(`comments/${commentId}/pin`, {
      method: "PATCH",
    });
  }

  async unpinComment(commentId: string): Promise<void> {
    return this.makeVoidRequest(`comments/${commentId}/unpin`, {
      method: "PATCH",
    });
  }
  ...
  async listTodos(params?: {
    ...
    if (params?.assignee_id)
      q.append("filter[assignee_id]", params.assignee_id);
    if (params?.status) q.append("filter[status]", params.status.toString());
    if (params?.limit) q.append("page[size]", params.limit.toString());
    if (params?.page) q.append("page[number]", params.page.toString());
    const qs = q.toString();
    return this.makeRequest<ProductiveResponse<ProductiveTodo>>(
      `todos${qs ? `?${qs}` : ""}`,
    );
  }
  ...
  async listPages(params?: {
    ...
    if (params?.page) q.append("page[number]", params.page.toString());
    const qs = q.toString();
    return this.makeRequest<ProductiveResponse<ProductivePage>>(
      `pages${qs ? `?${qs}` : ""}`,
    );
  }
  ...
  async listTaskDependencies(params?: {
    ...
    if (params?.task_id) q.append("filter[task_id]", params.task_id);
    if (params?.dependent_task_id)
      q.append("filter[dependent_task_id]", params.dependent_task_id);
